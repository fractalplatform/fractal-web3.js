...
*/
export async function sendSingleSigTransaction(txInfo, signInfo, parentIndex) {
  const multiSigInfos = [{ signInfo, indexes: [0]}];
  return sendMultiSigTransaction(txInfo, multiSigInfos, parentIndex);
}
...
*/
export async function sendMultiSigTransaction(txInfo, multiSignInfos, parentIndex) {
  await packTx(txInfo);
