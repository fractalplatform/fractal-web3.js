...
export async function sendSingleSigTransaction(txInfo, signInfo) {
  ...
  sendMultiSigTransaction(txInfo, multiSigInfos);
}
...
export async function sendMultiSigTransaction(txInfo, multiSignInfos) {
  ...
  let rlpData = encode([txInfo.gasAssetId, txInfo.gasPrice, [[action.actionType, action.nonce, action.assetId, action.accountName, action.toAccountName, action.gasLimit, action.amount, action.payload, action.remark, [...action.signData]]]]);
